...
DWORD LaunchProcess(PSID PackageSid)
{
    ...
    HANDLE hDeviceHandles[2] = { INVALID_HANDLE_VALUE, INVALID_HANDLE_VALUE };

    hDeviceHandles[0] = CreateFile(
        L"\\\\.\\CodeGateDevice",
    ...
    if (hDeviceHandles[0] == INVALID_HANDLE_VALUE) {
        Result = GetLastError();
        wprintf(L"Failed to open first device handle: %d\r\n", Result);
        goto Cleanup;
    }


    hDeviceHandles[1] = CreateFile(
        L"\\\\.\\CodeGateDevice",
        GENERIC_READ | GENERIC_WRITE,
        0,                    
        NULL,                 
    ...
    if (hDeviceHandles[1] == INVALID_HANDLE_VALUE) {
        Result = GetLastError();
        wprintf(L"Failed to open second device handle: %d\r\n", Result);
        goto Cleanup;
    }
    ...
    if (!SetHandleInformation(hDeviceHandles[0], HANDLE_FLAG_INHERIT, HANDLE_FLAG_INHERIT) ||
        !SetHandleInformation(hDeviceHandles[1], HANDLE_FLAG_INHERIT, HANDLE_FLAG_INHERIT)) {
        Result = GetLastError();
        wprintf(L"Failed to set handle inheritance: %d\r\n", Result);
        goto Cleanup;
    }
    ...
    if (NoWin32k == true)
    {
        ...
        ullMitigationPolicies[0] |= PROCESS_CREATION_MITIGATION_POLICY_STRICT_HANDLE_CHECKS_ALWAYS_OFF;
    ...
    {
        Result = GetLastError();
        wprintf(L"Failed to update thread attribute list with handle list: %d\r\n", Result);
        goto Cleanup;
    }
    ...
    wprintf(L"Inherited device handles to child process\r\n");
    CloseHandle(hDeviceHandles[0]);
    CloseHandle(hDeviceHandles[1]);
